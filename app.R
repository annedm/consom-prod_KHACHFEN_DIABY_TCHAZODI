...
shinyApp(
        ...
        tabItem("consommationinf36",align="center",
                column(width = 6, align="center",
                       box(
                         ...
                         column(10,align="center",style=list("padding-right: 3px;"),
                                selectInput(inputId = "Region_inf", label = "Sélectionner le national ou une région", choices= c(unique(data_inf_verticale$Region),"National") )),
                         
                         column(5,align="center",style=list("padding-right: 3px;"),
                                selectInput(inputId = "Profil_inf", label = "Profil", choices= c(unique(data_inf_verticale$Profil),"Tous les profils" ))),
        ...
        tabItem("production",align="center",
                column(width = 6,align="center",
                       box(
                         ...
                         column(10,align="center",style=list("padding-right: 3px;"),          
                                selectInput(inputId = "nat_reg_prod", label = "Sélectionner le national ou une région", choices= c(unique(data_11_verticale$Region), "National") )),
                         
                         column(5,align="center",style=list("padding-right: 3px;"),           
                                selectInput(inputId = "filiere_prod", label = "Filiere", choices= c(unique(data_11_verticale$Filiere.de.production), "Toutes les filieres" ) )),
  ...
  server = function(input, output, session) {
    ...
    construit_sup <- reactive({
      ...
      {
        new_data1 <- data_sup_verticale  %>% filter(Secteur.activite == input$secteur_activite_sup, variable %in% input$cat_sup, Plage.de.puissance.souscrite == input$plage_puissance_sup, Horodate <= as.POSIXct(input$dates_sup[2]) & Horodate >= as.POSIXct(input$dates_sup[1])) %>% group_by(Horodate,  variable) %>% summarise(value = sum(value))
    ...
    data3 <- eventReactive(input$buttonsup, {
